...
def _(students):
    ...
    print(f"Subset of students dataset with selected columns:")
    subset
    return
...
def _(pl, students):
    ...
    print(f"Students dataset with performance categories:")
...
def _(sales):
    ...
    print("Number of transactions:", sales.shape[0])
...
def _(pl, sales):
    ...
    # Then extract the month and create a new column "month" and display "month" in word

    sales_with_month = sales.with_columns(
        pl.col("date").str.strptime(pl.Date)
    ).with_columns(
        pl.col("date").dt.month().alias("month"),
        pl.col("date").dt.strftime("%B").alias("month_name") #Extra
    )

    print(f"Sales dataset with month column:")
    sales_with_month
